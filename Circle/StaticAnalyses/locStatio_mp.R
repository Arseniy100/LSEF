...
#            
# (2) B_source = "external_EnKF" # "internal_PMT"  "external_KF" --
#   specifies the source of input spatial covariances:
...
#   ="external_KF" -- KF's B matrices are read from a file and used to generate the truth and the ensm
#   ="external_EnKF" -- EnKF's S B matrices are read from a file and used to generate the truth and the ensm
#                       
...
#    (computed at a number,  n_repl,  of time instants during KF/EnKF filtering)
#    --- both for learning and computing verifying analyses.
#    
#  if B_source_KF_EnKF_spat_ave=F, then full KF/EnKF cvms are used as follows:
#    (1) If LEARN=T, LSM is fitted at each i_repl=1:n_repl  
#                    and the resulting local spectra and B_LSM are used as the TRUTH
#    (2) If LEARN=F,  B_KF_EnKF is used at each i_repl to generate the truth, ensm, obs.
...
#--------------------------------------------------------------
# Read  B  from an external source: KF or EnKF.
...
# This are estimated in the loop over n_repl below --- by fitting LSM
  ...
  if(B_source == "external_KF"){
    ...
    B_KF_EnKF             = KF_data$B_KF          # [1:nx, 1:nx, 1:ntime_B_store]
    ...
    # EnKF_data = list(SS_lcz, S_mean, b_shape_EnKF, spat_ave_cvfs_EnKF)
    
    B_KF_EnKF             = EnKF_data$SS_lcz             # [1:nx, 1:nx, 1:ntime_B_store]
  ...
  if(B_source_KF_EnKF_spat_ave){  # take cvfs (space-statio) 
    true_spectra_available = TRUE
    ...
    spat_ave_cvfs_KF_EnKF_thinned = spat_ave_cvfs_KF_EnKF[,ind]
    ntime_B_KF_EnKF = dim(spat_ave_cvfs_KF_EnKF_thinned)[2]
    
  }else{                          # take cvms (space-nstatio)
    if(is.null(B_KF_EnKF)){
      message("B_KF_EnKF were not stored in the LSEF run. Set ntime_B_store>0 in config.txt there.")
      stop
    }else{
      ntime_B_KF_EnKF = dim(B_KF_EnKF)[3]
    }
    
    if(LEARN){                        # b_LSM will be used as the truth
      true_spectra_available = TRUE
    }else{                            # B_KF_EnKF is the truth, no local spectra
...
if(LEARN){
  ...
  if(B_source == "internal_PMT"){
    ...
    n_learningSample = n_repl # if external B_KF_EnKF cvfs are used, take one cvf per n_repl
  }
...
if(B_source == "internal_PMT"){
  ...
  if(i_repl <= n_repl_CC) Cv = Cov2VarCor(B)
 
#-------------------------
  
}else{ # B_source = "external_KF"  or  "external_EnKF" (treated identically)
  
  if(B_source_KF_EnKF_spat_ave){  # take spat-statio  cvfs
    ...
    if(LEARN){   # fit  LSM  usig SVshape to get  b_true, recalc B, etc as usual
      B = B_KF_EnKF[,,i_repl]
      
      perform_B2S = T
      LSM_KF_EnKF = fit_LSM(B, ne, tranfu, BOOTENS, nB, true_spectra_available=F,
...
C = Cv$C
if(i_repl <= n_repl_CC) CC[,,i_repl] = C
...
# Fit LSM: 
# 1) Generate the ENSM
# 2) E2B (bafrom tyhe ENSM to sample abnd variances)
# 3) B2S

perform_B2S = !LEARN # don't perform B2S if LEARN=T

LSM = fit_LSM(B, ne, tranfu, BOOTENS, nB, true_spectra_available,
