...
lin_determ_anls = function(X_f, X_obs, H, B_specified, R, B_true){
  #-------------------------------------------------------------------------------------------
  # Perform multiple linear deterministic analyses with different
  # sets of FG & OBS but with the same obs oprt H and
  ...
  # B_specified - available (used in the anls) FG-err CVM
  ...
  K = B_specified %*% t(H) %*% solve(H%*% B_specified %*% t(H) + R)
